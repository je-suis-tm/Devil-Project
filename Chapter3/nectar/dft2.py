...
def initialize_agents(num_of_steps, num_of_agents, all_coordinates, min_fitness, max_fitness, max_age, seed_num=None):
    ...
    metrics=['roadkill', 'starvation', 'monopoly', 'natural mortality','newborn',
             ...
             'dft2 susceptible','dft2 exposed','dft2 infected','average fitness']
...
def run_simulation(num_of_agents, num_of_steps, age_prob, mating_period,
                   ...
                   dft2_intro,max_dft2_transmission_rate,max_dft2_latency_period,min_dft2_latency_period,dft2_fitness_decline_rate,
                   seed_num=None):
...
#         dft1_map_denominator=np.zeros(len(all_coordinates))
#         dft1_map_numerator=np.zeros(len(all_coordinates))
#         for agent_ind in alive_agents:
#             dft1_map_denominator[all_coordinates.index(agent_loc[agent_ind])]+=1
#         for agent_ind in dft1_infected_mat[-1]:
#             dft1_map_numerator[all_coordinates.index(agent_loc[agent_ind])]+=1
...
#         dft2_map_denominator=np.zeros(len(all_coordinates))
#         dft2_map_numerator=np.zeros(len(all_coordinates))
#         for agent_ind in alive_agents:
#             dft2_map_denominator[all_coordinates.index(agent_loc[agent_ind])]+=1
#         for agent_ind in dft2_infected_mat[-1]:
#             dft2_map_numerator[all_coordinates.index(agent_loc[agent_ind])]+=1
...
with concurrent.futures.ProcessPoolExecutor(max_workers=num_parallel) as executor:
    ...
    for future in concurrent.futures.as_completed(futures):
        i, stats, _, _, _, _, dft1_prognosis, _, _, _,dft2_prognosis,_ = future.result()
        stats_dict[i] = stats
        dft1_prognosis_dict[i] = dft1_prognosis
        dft2_prognosis_dict[i] = dft2_prognosis
